...
class StructuredPerceptron:
    def __init__(self, trainfilepath):
        ...
        self.iterations = 1 # number of iterations to run through all the sentences
        self.traindata = clean_trainset(trainfilepath)
        self.testfilepath = testfilepath
        self.tags = self.traindata.get_tags()
        self.tags = ['A', 'P', 'V', 'D', 'N', 'START']
    ...
    def train(self):
        ...
        traindata = [[('', 'START'), ('time', 'N'), ('flies', 'V'), ('like', 'P'), ('an', 'D'), ('arrow', 'N')]]
    ...
    def updateweights(self, predtagsentence, actualtagsentence, predtagtransitions, actualtagtransitions,
        ...
        for tag in goldfeaturevec.keys():
            if tag in predfeaturevec.keys():
                self.weights[tag] = self.weights[tag] + (goldfeaturevec[tag] - predfeaturevec[tag]) * learning_rate
            else:
                self.weights[tag] = self.weights[tag] + goldfeaturevec[tag] * learning_rate
        ...
        for tag in predfeaturevec.keys():
            ...
            if tag in goldfeaturevec.keys():
                ...
                self.weights[tag] = self.weights[tag] - predfeaturevec[tag] * learning_rate
...
trainfilepath = 'Test/train'
testfilepath = 'Test'
perceptron = StructuredPerceptron(trainfilepath)
perceptron.train()
#print(perceptron.weights)
perceptron.predict(testfilepath)
#print(perceptron.get_structured_perceptron_path(['', 'time', 'flies', 'like', 'an', 'arrow']))
